...
Dice.prototype.clone = function() {
    return new Dice([this.top, this.back, this.right, this.left, this.front, this.bottom]);
};
...
(function(input) {
    var p = input.split('\n');
    var labels1 = p.shift().split(' ').map(Number);
    var labels2 = p.shift().split(' ').map(Number);
    var dice1 = new Dice(labels1);
    var dice2 = new Dice(labels2);
    console.log(dice1.isEqualLabels(dice2) ? 'Yes' : 'No');
